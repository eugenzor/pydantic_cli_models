...
def test_format_binary_content_with_text_only():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        messages = [
            ModelRequest(parts=[UserPromptPart(content="Just a simple text message")])
        ]
...
def test_format_binary_content_identifier_sanitization():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        binary_content = BinaryContent(
            data=png_data, media_type="image/png", identifier="my/unsafe:file<name>"
        )

        messages = [
            ModelRequest(
                parts=[UserPromptPart(content=["Check this:", binary_content])]
            )
        ]
