...
class TestFileHandler:
    ...
    def test_read_structured_output_success(self, tmp_path):
        ...
        with patch(
            "pydantic_ai_claude_code.structured.file_handler.read_structure_from_filesystem"
        ) as mock_read:
...
class TestFunctionSelector:
    ...
    def test_build_function_selection_prompt_includes_none_option(self):
        ...
        assert (
            "Answer directly without calling a function" in prompt
            or "without" in prompt.lower()
        )
    ...
    def test_build_argument_collection_prompt_structure(self):
        ...
        with patch(
            "pydantic_ai_claude_code.structured.function_selector.build_structure_instructions"
        ) as mock_build:
            mock_build.return_value = "Structure instructions"

            prompt = build_argument_collection_prompt(
                "get_weather", "Get weather information", schema, "/tmp/output"
            )
    ...
    def test_build_retry_prompt_includes_error(self):
        ...
        with patch(
            ...
            prompt = build_retry_prompt(
                "original prompt", schema, "/tmp/output", error_msg
            )
    ...
    def test_build_retry_prompt_rebuilds_instructions(self):
        ...
        with patch(
            ...
            prompt = build_retry_prompt("original", schema, "/tmp/new_output", "error")
