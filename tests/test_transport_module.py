...
class TestEnhancedCLITransport:
    ...
    async def test_execute_setup_working_directory(self):
        ...
        with patch.object(
            transport, "_setup_working_directory", return_value="/tmp/test"
        ), patch.object(transport, "_build_command", return_value=["claude"]):
            with patch.object(
                transport,
                "_try_execution_with_retry",
    ...
    async def test_execute_handles_oauth_error(self):
        ...
        with patch.object(
            ...
            with patch.object(
                transport,
                "_try_execution_with_retry",
                side_effect=ClaudeOAuthError("Token expired"),
            ):
    ...
    async def test_execute_retries_infrastructure_failures(self):
        ...
        with patch.object(
            ...
            with patch.object(
                transport,
                "_try_execution_with_retry",
                side_effect=mock_try_execution,
            ):
    ...
    def test_setup_working_directory_creates_temp(self, tmp_path):
        ...
        with patch("tempfile.mkdtemp", return_value=mock_dir), patch(
            "pydantic_ai_claude_code.transport.sdk_transport.save_prompt_debug"
        ):
    ...
    def test_setup_working_directory_uses_existing(self):
        ...
        transport = EnhancedCLITransport(
            "test prompt", {"working_directory": "/custom/dir"}
        )
    ...
    def test_build_command_with_sandbox(self):
        ...
        with patch(
            ...
            "pydantic_ai_claude_code.transport.sdk_transport.wrap_command_with_sandbox"
        ) as mock_wrap:
    ...
    async def test_try_execution_with_retry_rate_limit(self):
        ...
        with patch.object(
            transport, "_execute_command", side_effect=mock_execute_command
        ), patch.object(transport, "_classify_error") as mock_classify:
...
class TestIntegration:
    ...
    async def test_full_execution_flow_success(self):
        ...
        settings = {"model": "sonnet", "working_directory": "/tmp/test"}
        ...
        with patch(
            "pydantic_ai_claude_code.utils_legacy.resolve_claude_cli_path",
            return_value="/usr/bin/claude",
        ), patch("tempfile.mkdtemp", return_value="/tmp/mock"):
            with patch("pathlib.Path.mkdir"):
                with patch("pathlib.Path.iterdir", return_value=[]):
                    with patch("pathlib.Path.write_text"):
                        mock_process = AsyncMock()
