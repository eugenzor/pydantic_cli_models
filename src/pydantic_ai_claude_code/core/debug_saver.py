...
# Global counter for sequential numbering of prompts/responses
# Protected by _counter_lock to ensure thread safety
_debug_counter = 0
...
def save_prompt_debug(prompt: str, settings: dict[str, Any] | None) -> None:
    ...
    The counter is thread-safe: it uses a lock to ensure each prompt gets a unique counter value, which is then stored in settings['__debug_counter'] so the corresponding response can use the same counter value.

    Parameters:
        prompt (str): Prompt text to save.
        settings (dict[str, Any] | None): Settings mapping; debug saving is enabled when settings["debug_save_prompts"] is truthy. If this key is True the default debug directory is used, otherwise its string value is treated as the debug directory path. The counter value is stored in settings['__debug_counter'] for use by save_response_debug.
    """
    ...
    # Store counter in settings for the corresponding response
    settings["__debug_counter"] = counter_value

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = f"{counter_value:03d}_{timestamp}_prompt.md"
...
def save_response_debug(
    ...
    This function reads the counter value from settings['__debug_counter'] that was set by save_prompt_debug, ensuring the response file is correctly paired with its prompt file even in concurrent environments.

    Parameters:
        response (dict[str, Any]): The response object to serialize and save.
        settings (dict[str, Any] | None): Application settings used to determine the debug directory (e.g., the `debug_save_prompts` setting). Should contain '__debug_counter' set by save_prompt_debug.
    """
    debug_dir = get_debug_dir(settings)
    if not debug_dir:
        return
